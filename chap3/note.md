...
└─$ ps -eo pid,ppid,sid,tty,pgrp,stat（显示进程状态）,comm | grep -E zsh
  ...
  97589   87056   97589 pts/1      97589 Ss（这里的小写s，就是session leader的意思）
```
...
pid 有限，有可能 pid 耗尽了。32 位的机器上：最大 pid 是 32767
...
## chap3 - 07 守护进程详解、范例演示

### 普通进程运行观察

1. 进程有对应的终端，如果终端退出，对应的进程也会退出：它的父进程是一个 bash
2. 终端被占用了：输入各种命令，终端都没有反应

### 守护进程基本概念

守护进程：一种长期运行的进程：这种进程在后台运行，并且不跟任何控制终端关联

基本特点：

1. 生存期长：一般是操作系统启动的时候他就启动，操作系统关闭的时候他也关闭
2. 守护进程跟终端无关联，也就是说他们没有控制终端，所以控制终端退出，守护进程并不会退出
3. 守护进程是在后台运行，不会占用终端，终端可以执行其他命令

linux 操作系统本身是有很多的守护进程在默默运行，维持着系统的日常活动。大概 30~50 行
...
root           1       0       1       1  0 Jan07 ?        00:00:04 /sbin/init splash
root           2       0       0       0  0 Jan07 ?        00:00:00 [kthreadd] # 这种带[]就是内核守护进程
```

1. ppid = 0，属于内核进程，跟随系统启动而启动，生命周期贯穿整个系统
2. CMD 列，名字带[]的，是内核守护进程
3. 老祖 init，也是系统守护进程，负责启动各运行层次特定的系统服务; 所以很多进程的 ppid 是 init; 并且还会收养孤儿进程
4. CMD 列，名字不带[]的，是普通守护进程，或者也是用户级守护进程
5. 许多守护进程是：进程组的组长，或者是会话的首进程，而且很多是自成一组，或者是会话中唯一的进程

共同特点：

1. 大多数守护进程都是以 超级用户特权 运行的的
2. 守护进程没有控制终端，也就是 TT 这列显示`?`
   - 内核守护进程以无控制终端方式启动
...
这个 umask(0) 应该是 umask(000)，对应的权限就是：777-000=777

umask 是一个函数，用来限制（屏蔽）一些文件权限的。
设置为 0，就是关闭这种限制功能的

2. fork() 一个子进程出来，然后父进程退出：固定套路

父进程退出，终端就不会被占用了 --> 把终端空出来

而且之前也讲过：父进程的 setsid 是没有效果的。

setsid 是为了让：子进程有单独的 sid：而且子进程也成为了一个新进程组的组长进程;
同时，子进程不关联任何终端了
...
当你打开了一个存在的文件或者创建一个新文件，
操作系统都会返回这个文件描述符，
后续对这个文件的操作的一些函数，都会用到这个文件描述符

linux 中 3 个特殊的文件描述符，数字分别为 0，1，2

// 0：一般表示标准输入 —— 键盘，对应的符号常量叫 STDIN_FILENO
// 1：一般表示标准输出 —— 屏幕，对应的符号常量叫 STDOUT_FILENO
// 2：一般表示标准错误 —— 屏幕，对应的符号常量叫 STDERR_FILENO

其实不完全吧，毕竟我在看 tlpi（应该是老师为了便于学生理解）

unix 哲学：一切皆文件。 也就是：
像看待文件一样看待：stdin，stdout，stderr;
像操作文件一样操作：stdin，stdout，stderr

同时，程序一旦与虚拟国内起来，这三个文件描述符 0,1,2 会被自动打开（自动指向对应的设备）

#### 输入输出重定向

#### 空设备

`/dev/null` 是一个特殊的设备，用来丢弃一切写入其中的数据

守护进程通过终端启动，但是和终端不挂钩。
守护进程是在后台运行，它不应该从键盘上接受任何东西， 也不应该把输出结果打印到屏幕或者终端上。
所以，一般按照江湖规矩，我们要把守护进程的 stdin，stdout，重定向到`/dev/null`。
...
if (fd > STDERR_FILENO) {
    close(fd);  // 相当于把fd指针置为NULL，释放资源
}
```

dup2 的官方解释是：复制文件描述符。但是这个感觉就像是：指针赋值一样，实际上也差不多，把句柄指向啥

#### 实现范例 （可做参考）

![创建守护进程](./nginx3_7_2.c)

守护进程可以用命令启动，如果想要开机启动，则需要借助 系统初始化脚本来启动（可能是 systemctl 之类的）

### 守护进程不会收到的信号

#### sighup 信号

守护进程不会收到来自内核的 sighup 信号;
潜台词：如果守护进程收到了 sighup 信号，那么肯定是另外的进程发送给守护进程的

很多守护进程会把 sighup 信号作为通知信号：表示配置文件已经发生改动，守护进程应该重新读入其配置文件

#### sigint、sigwinch 信号

sigint 是：终端 中断符号`ctrl + c`。
既然守护进程已经与终端脱钩了，那么自然这种 sigint 信号就可以用来做我们自己的事情了，
相当于多了一个可自定义的信号
...
1. 守护进程和终端不挂钩。后台进程依然能往终端上输出东西（和终端挂钩）
